...
service Zones {
  ...
  rpc Arm(ArmRequest) returns (ArmedZones) {
    option (google.api.http) = { post: "/api/zones/arm" body: "*" };
  }
}
```
...
MATLAB's `webread`. The `google.api.http` option binds each rpc to the REST route
it actually rides, and that route is the only way to call it.
...
|---|---|---|
| camelCase | `position_cm` → `positionCm` | `json_name` on every field, without exception |
...
| `make check-proto` | fail when the committed generated code is not what the `.proto` produces |
